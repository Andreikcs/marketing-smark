...
11. **Acervo, seed e custo.** Quem garante a consistência visual é o **acervo** (`scripts/acervo.py`): ele guarda as peças aprovadas de cada família e as injeta como `input_references` nas gerações seguintes — só entra peça que passou no `revisar.py` e foi marcada à mão (ou via botão ★ no Super Editor). Cada arte aprovada deixa a próxima mais parecida com a marca; é o único ativo do sistema que não dá pra copiar. A **seed** (`família + slug + tipo`) é gravada nos metadados e serve de rótulo e de `--reroll N`, mas **não é garantia de reprodutibilidade**: o modelo default aceita `seed` e ignora. Não prometa "mesma seed, mesma imagem". Custo de cada geração fica em `design-system/custos/geracoes.jsonl`.
